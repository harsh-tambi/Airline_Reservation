...
def agenthome(request):
    ...
    cursor.close()
    return render(request, 'agent/agenthome.html', {'info': upcoming_flights, 'info_delayed': info_delayed})
...
def agentpurchaseticket(request):
    if request.method == 'POST':
        ...
        print (bool(name_of_airline == 'AirIndia'))
        flight_num = x[1][1:]
        print (flight_num)
        email = request.POST['email']
