...
def staffnewsletter(request):
    if request.method == 'POST':
        cursor = conn.cursor()
        email = request.POST['email']
        q = "SELECT * FROM newsletter WHERE email = \"{}\" ".format(email)
        cursor.execute(q)
        data = cursor.fetchone()
        if not data:
            q = "INSERT INTO newsletter VALUES (\"{}\") ".format(email)
            cursor.execute(q)
            conn.commit()
...
def topdestin(request):
    ...
    cursor.close()
    cursor = conn.cursor()
    month = "SELECT a.city FROM bookings b, ticket t, flight f, airport a WHERE t.ticket_id = b.ticket_id AND t.name_of_airline = f.name_of_airline AND t.flight_num = f.flight_num AND f.name_of_airline = \'{}\' AND (f.arrival_time BETWEEN ADDDATE(NOW(), INTERVAL -3 MONTH) AND NOW()) AND a.name = f.arrival_airport_name GROUP BY f.arrival_airport_name, a.city ORDER BY COUNT(b.email_customer) DESC "
    cursor.execute(month.format(staff_airl[0]))
    top_d = []
    d = cursor.fetchone()
    count = 1
    while d and count <= 3:
        ...
        count += 1
    cursor.close()
    lst_3m = [a for a in top_d]
    cursor = conn.cursor()
    year = "SELECT a.city FROM bookings b, ticket t, flight f, airport a WHERE t.ticket_id = b.ticket_id AND t.name_of_airline = f.name_of_airline AND t.flight_num = f.flight_num AND f.name_of_airline = \'{}\' AND (f.arrival_time BETWEEN ADDDATE(NOW(), INTERVAL -12 MONTH) AND NOW()) AND a.name = f.arrival_airport_name GROUP BY f.arrival_airport_name, a.city ORDER BY COUNT(b.email_customer) DESC "
    cursor.execute(year.format(staff_airl[0]))
    top_d = []
    d = cursor.fetchone()
    count = 1
    while d and count <= 3:
        ...
        count += 1
    cursor.close()
    lst_1y = [a for a in top_d]
    print (lst_1y)
    print (lst_3m)
    return render(request, 'staff/topdestin.html', {'lst_1y': lst_1y, 'lst_3m':lst_3m})
...
def newpermission(request):
    ...
    if (bool(admin_permission[0])):
        ...
        return render(request, 'staff/newpermission.html', {'employees': employees[0]})
    else:
        redirect('staffhome')
